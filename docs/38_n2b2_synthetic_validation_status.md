...
Bundle、App、模型下载/替换、G1 renewal 与全量夜间流水线。将经验证的源码和脱敏文档
线性整合到 `main` 不等于生产能力启用，也不改变任何上述禁止项。
...
- N2B1P manifest portability remediation 解决了不同 Windows checkout 的清单字节差异；
  它仍要求生产 N2B2 保持锁定。
- N2B2 的 GPU、Qwen、S3、S20、bundle 与恢复验证均是 synthetic-only 证据；历史失败
  （fixture capability、Case 17、Qwen digest 与终端 artifact 一致性）仍保留为诊断
  记录，而不是被改写成成功。
- 当前主线仅等待独立外部 Review。该审查必须针对最终线性候选和 Git 外、已校验的
  synthetic evidence；未通过前不得创建 Real20 execution lease，也不得读取真实照片。
