...
from nightly_photo_intelligence_pipeline.engineering.geometry import (
    MAX_BOXES,
    measure_bbox_empty_area,
)
...
class NegativeSpaceTests(unittest.TestCase):
    ...
    def test_duplicate_boxes_do_not_change_measurement(self):
        self.assertEqual(
            ...
            measure_bbox_empty_area([box(), box()], 100, 100),
        )

    def test_clips_at_frame(self):
        self.assertEqual(
            measure_bbox_empty_area([box(-10, -10, 20, 200)], 100, 100),
            measure_bbox_empty_area([box()], 100, 100),
        )

    def test_offscreen_and_zero_area(self):
        self.assertEqual(
            measure_bbox_empty_area([box(200, 0, 300, 10), box(0, 0, 0, 5)], 100, 100),
    ...
    def test_random_union_against_independent_pixel_oracle(self):
        ...
        for _ in range(100):
            ...
            free = {
                ...
                for y in range(10)
                if not any(
                    b["x_min"] <= x < b["x_max"] and b["y_min"] <= y < b["y_max"] for b in boxes
                )
            }
