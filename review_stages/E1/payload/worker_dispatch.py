...
MODES = {"fresh", "resume"}
CONFIG_FIELDS = {
    "project_root", "cache_root", "s3_manifest_dir", "s20_manifest_dir",
    "baseline_manifest_dir", "s3_out", "s20_out", "prior_s20_review_record",
    "reviewed_commit", "candidate_tree", "source_manifest_sha256",
    "ledger_root", "reservation_dir", "receipt_sha256", "bindings_sha256",
    "runtime_identity_sha256",
}
PATH_FIELDS = {
    "project_root", "cache_root", "s3_manifest_dir", "s20_manifest_dir",
    "baseline_manifest_dir", "s3_out", "s20_out", "prior_s20_review_record",
    "ledger_root", "reservation_dir",
}
...
def _read(path: Path) -> bytes:
    ...
    _require(stat.S_ISREG(before.st_mode) and before.st_nlink == 1
             and before.st_size <= MAX_BYTES, "NPI_DISPATCH_RECORD_INVALID")
...
def _configuration(configuration: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for key in PATH_FIELDS:
        _path(result[key])
    for key in ("receipt_sha256", "bindings_sha256", "source_manifest_sha256", "runtime_identity_sha256"):
    ...
    _require(_path(result["reservation_dir"]) == _path(result["ledger_root"]) / result["receipt_sha256"],
...
def _reservation(configuration: Mapping[str, Any]) -> tuple[Path, str]:
    ...
    _require(record["schema_version"] == "npi-lease-consumption-v1" and record["status"] == "RESERVED"
...
def _fresh_complete(root: Path, configuration_sha: str) -> None:
    ...
    _require(isinstance(value, dict) and value.get("outcome") == "COMPLETE"
             and value.get("configuration_sha256") == configuration_sha
             and _hex(value.get("result_sha256")), "NPI_WORKER_FRESH_NOT_COMPLETE")
...
def issue(configuration: Mapping[str, Any], mode: str) -> dict[str, Any]:
    ...
    record = {
        "schema_version": "npi-worker-dispatch-v1", "mode": mode,
        "configuration_sha256": config_sha, "reservation_sha256": reservation_sha,
        "nonce_sha256": _sha(nonce.encode("ascii")),
    }
    _write_new(root / f"worker-{mode}-dispatch.json", record)
    return {"schema_version": "npi-worker-envelope-v1", "mode": mode,
            "configuration": config, "nonce": nonce}
...
def claim(envelope: Mapping[str, Any], mode: str) -> dict[str, Any]:
    ...
    _require(mode in MODES and set(envelope) == {"schema_version", "mode", "configuration", "nonce"},
             "NPI_WORKER_ENVELOPE_INVALID")
    _require(envelope["schema_version"] == "npi-worker-envelope-v1" and envelope["mode"] == mode
             and _hex(envelope["nonce"]) and isinstance(envelope["configuration"], Mapping),
             "NPI_WORKER_ENVELOPE_INVALID")
    ...
    _require(_json(_read(root / f"worker-{mode}-dispatch.json")) == expected,
...
def finish(configuration: Mapping[str, Any], mode: str, *, outcome: str, result: object) -> None:
    ...
    _require(isinstance(record, dict) and record.get("configuration_sha256") == config_sha,
             "NPI_WORKER_CLAIM_MISMATCH")
    _write_new(root / f"worker-{mode}-result.json", {
        "outcome": outcome, "configuration_sha256": config_sha,
...
def assert_identity(configuration: Mapping[str, Any], identity: Mapping[str, Any]) -> None:
    ...
    _require(_sha(_canonical(dict(identity))) == configuration.get("runtime_identity_sha256"),
             "NPI_WORKER_RUNTIME_IDENTITY_DRIFT")


def assert_source(configuration: Mapping[str, Any], source: Mapping[str, Any]) -> None:
    _require(configuration.get("reviewed_commit") == source.get("candidate_commit")
