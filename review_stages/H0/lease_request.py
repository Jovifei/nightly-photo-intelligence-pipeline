...
def load_path_plan(path: Path) -> tuple[dict[str, dict[str, str]], str]:
    ...
    require(isinstance(payload, dict) and set(payload) == set(REQUIRED_PATH_PLAN), "H0_PATH_PLAN_INVALID")
...
def build_request(args: argparse.Namespace) -> dict[str, Any]:
    ...
    review_sha = digest_file(Path(args.review_artifact), "H0_REVIEW_ARTIFACT_MISSING")
    ...
    require(HEX64.fullmatch(args.model_cache_binding_sha256) is not None, "H0_CACHE_BINDING_INVALID")
...
def main(argv: list[str] | None = None) -> int:
    try:
        ...
        print(json.dumps({"result": "H0_REQUEST_INVALID", "error": str(exc), "execution_authorized": False}))
        return 1
