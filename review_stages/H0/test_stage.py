...
class LeaseRequestTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.tree = self.git_out("rev-parse", "HEAD^{tree}")
        ...
        self.historical_review = self.root / "historical-review.md"
        self.historical_review.write_text("INCONCLUSIVE\n" + lr.HISTORICAL_BLOCKER + "\n")
        self.prior_review = self.root / "prior-s20-review.json"
        self.prior_review.write_text('{"reviewed_commit":"' + "c" * 40 + '"}\n')
        self.quality = self.root / "quality.json"
        self.quality.write_text('{"complete":true}\n')
        ...
        self.s3.write_text("s3\n")
        self.s20 = self.root / "s20.json"
        self.s20.write_text("s20\n")
        ...
        self.path_plan = self.root / "path-plan.json"
        self.path_plan.write_bytes(lr.canonical(plan))
        receipt = self.repo / lr.HISTORICAL_RECEIPT
        receipt.parent.mkdir(parents=True)
        receipt.write_text(
            'external_review_sha256: "' + lr.sha(self.historical_review.read_bytes()) + '"\n'
        )
        self.git("add", ".")
        self.git("commit", "-qm", "bind historical review")
        self.head = self.git_out("rev-parse", "HEAD")
        self.tree = self.git_out("rev-parse", "HEAD^{tree}")
        self.candidate_review.write_bytes(
            lr.canonical(
    ...
    def args(self) -> argparse.Namespace:
        return argparse.Namespace(
            ...
            expected_candidate=self.head,
            candidate_review_artifact=self.candidate_review,
            historical_review_artifact=self.historical_review,
            prior_s20_review=self.prior_review,
            quality_evidence=self.quality,
            runtime_identity=self.identity,
            s3_manifest=self.s3,
            s20_manifest=self.s20,
            model_cache_binding_sha256="b" * 64,
    ...
    def test_builds_draft_only_and_binds_source(self) -> None:
        ...
        self.assertEqual(lease["schema_version"], "npi-synthetic-execution-lease-v3")
        self.assertEqual(lease["bindings"]["candidate_commit"], self.head)
        self.assertEqual(
            lease["bindings"]["source_manifest_sha256"],
            packet["candidate"]["source_manifest_sha256"],
        )
        self.assertEqual(
            lease["bindings"]["candidate_review_sha256"],
            lr.sha(self.candidate_review.read_bytes()),
        )
        self.assertEqual(
            lease["bindings"]["historical_review_sha256"],
            lr.sha(self.historical_review.read_bytes()),
        )
        self.assertEqual(
            lease["bindings"]["prior_s20_review_sha256"],
            lr.sha(self.prior_review.read_bytes()),
        )
        self.assertEqual(
            lease["bindings"]["quality_evidence_sha256"],
            lr.sha(self.quality.read_bytes()),
        )
    ...
    def test_n2b2_must_remain_locked(self) -> None:
        ...
        self.git("add", "PROJECT_STATE.json")
        self.git("commit", "-qm", "state")
        args = self.args()
        args.expected_candidate = self.git_out("rev-parse", "HEAD")
    ...
    def test_bad_cache_digest_rejected(self) -> None:
        args = self.args()
        args.model_cache_binding_sha256 = "bad"
        with self.assertRaisesRegex(ValueError, "H0_CACHE_BINDING_INVALID"):
            lr.build_request(args)

    def test_missing_distinct_review_inputs_are_rejected(self) -> None:
        args = self.args()
        args.historical_review_artifact = self.root / "missing-review.md"
        with self.assertRaisesRegex(ValueError, "H0_HISTORICAL_REVIEW_MISSING"):
            lr.build_request(args)

    def test_review_roles_are_semantically_bound(self) -> None:
        args = self.args()
        args.candidate_review_artifact, args.historical_review_artifact = (
            args.historical_review_artifact,
            args.candidate_review_artifact,
        )
        with self.assertRaisesRegex(ValueError, "H0_CANDIDATE_REVIEW_INVALID"):
            lr.build_request(args)
