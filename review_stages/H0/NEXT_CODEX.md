...
4. Prepare Git-external inputs only from already-known Owner-controlled locations:
   - independent review artifact for the current runtime candidate;
   - historical review artifact bound by the task-specific receipt;
   - prior S20 review record;
...
10. Return the request path + SHA-256, candidate/tree/source digest, state hash,
    all three review hashes, quality hash, runtime identity hash, fixture/cache/path
    hashes, and any blocker. Stop at `H1_DRAFT_REQUEST_READY_FOR_OWNER_REVIEW` or
    an honest blocked state.
