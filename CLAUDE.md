...
当前项目规划中，Claude Code 默认是**独立 Reviewer**。只有 Owner 明确将 Claude Code 指定为当前 N2B0.7 实现智能体时，才可实施该资格审查；即使如此，权限仍与 Codex 完全相同，不多也不少。
...
完成 N2B0.7 实现或审查后必须停止，等待 Owner artifact decision。
